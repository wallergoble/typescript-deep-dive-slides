...
<TypescriptCodeBlock>
  {`type UserProperties = 'firstName' | 'lastName' | 'email'\n
type User = Record<UserProperties, string>`}
</TypescriptCodeBlock>
<TypescriptCodeBlock>
{`const waller: User = {
  firstName: 'Waller',
...
What if I want to make a lot of properties generic?
...
- an attempted improvement on `any`
- forces the developer to check any usage of the value
...
<TypescriptCodeBlock>{`// @types/react/index.d.ts*`}</TypescriptCodeBlock>
